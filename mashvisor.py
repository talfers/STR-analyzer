...
class Mashvisor:
    ...
    def get_neighborhoods(self, city, state):
    ...
    def get_neighborhood_str_historical_performance(self, id, state, average_by, percentile, beds=None):
    ...
    def get_neighborhood_overview(self, id, state):
        response = requests.get(f"{self.base_url}/neighborhood/{id}/bar", headers=self.headers, params={"state":state})
        return self.return_response(response)
    
    
    def get_location_heatmap(self, map_type, state, ne_lat, sw_lng, ne_lng, sw_lat):
    ...
    def get_city_performance(self, city, state):
        response = requests.get(f"{self.base_url}/city/investment/{state}/{quote(city)}", headers=self.headers)
        return self.return_response(response)
    
    
    def get_rental_rates(self, state, source, city, zipcode=None):
